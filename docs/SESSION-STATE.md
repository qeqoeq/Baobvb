...
| B4 — Counterpart name absent | **EN COURS — diagnostic fait, fix SQL non écrit** | (1) `user_public_profiles` sans colonnes `display_name`/`handle` ; (2) RPC `my_shared_relationships()` ne retourne pas `counterpart_display_name` | À écrire en une seule passe après le résultat B8 : ALTER TABLE + CREATE OR REPLACE `upsert_user_handle` (sync display_name/handle) + jointure dans `my_shared_relationships()` | — | — |
...
| B8 — Doublons de claim | **EN COURS — bloqué STOP SQL** | Hypothèse : contrainte `UNIQUE(relationship_id)` absente sur `shared_relationship_reveals` — À CONFIRMER par le diagnostic | Diagnostic SQL préparé (voir SQL EN ATTENTE), fix SQL + client (bouton disabled pendant claim) à écrire selon résultat | — | — |
