...
export type Place = {
  ...
  wentAgainAt?: string;
  /**
   * Append-only accumulation of private reads over time (X.77). Never
   * mutated or removed once added — addPlaceRead is the only writer.
   * quickSignal/impression above remain the legacy first read and are
   * never migrated into this array; both coexist, with the private place
   * value input helper resolving which read snapshot is authoritative.
   */
  reads?: PlaceReadEntry[];
};

/**
 * A single, immutable private read captured at one point in time.
 * categorySnapshot/criteriaVersion exist so a future change to the
 * category or the criteria vocabulary never reinterprets an old entry —
 * each entry stays self-describing forever.
 */
export type PlaceReadEntry = {
  id: string;
  createdAt: string;
...
/**
 * Caller-provided shape for a new read. id/createdAt/categorySnapshot/
 * criteriaVersion are always generated by the store — never accepted from
 * the caller, so a read can never be backdated, mis-attributed, or have
 * its identity spoofed.
 */
export type PlaceReadEntryInput = {
  landingLevel?: PlaceLandingLevel;
  contextFit?: PlaceContextFit[];
  driverDimensions?: RestaurantExperienceDimension[];
  restaurantDimensions?: RestaurantExperienceDimensions;
...
/**
 * The decision the receiver made for a received object.
 * No 'seen', 'opened', 'pending', 'ignored', or 'expired' states —
 * silence is never penalized and delivery is never acknowledged.
 */
export type ReceivedObjectStatus = 'new' | 'kept' | 'not_for_me';

/**
 * A local, append-only record of an object received from someone who thought of you.
 * id/createdAt/objectType/status are always generated by the store.
 * Local only. Never sent to Supabase. No read receipt. No route score.
 * decidedAt is only populated when status transitions from 'new'.
 * fromDeliveryId links to the server-side pass_delivery row (idempotency key).
 */
export type ReceivedObject = {
  id: string;
  createdAt: string;
  objectType: 'place';
  objectId: string;
  ...
  note?: string;
  sourceRelationId?: string;
  status: ReceivedObjectStatus;
  decidedAt?: string;
};

/**
 * Caller-provided input for a new received object.
 * id/createdAt/objectType/status/decidedAt are always generated by the store.
 */
export type ReceivedObjectInput = {
  objectId: string;
  fromRelationId: string;
  fromDeliveryId?: string;
  nameSnapshot: string;
...
/**
 * Input shape for materializing remote pass deliveries as local ReceivedObjects.
 * Defined here to avoid a circular dependency with lib/pass-delivery-repo.
 */
export type PassDeliveryMaterializationInput = {
  fromDeliveryId: string;
...
/**
 * A local, append-only record that someone passed this place to a relation.
 * id/createdAt/objectType are always generated by the store — never accepted
 * from the caller, so a pass can never be backdated or spoofed.
 * No status, no receipt, no route score. Local only. Never sent to Supabase.
 */
export type PassedObject = {
  id: string;
  createdAt: string;
  objectType: 'place';
  objectId: string;
...
type StoreState = {
  ...
  places: Place[];
  passedObjects: PassedObject[];
  receivedObjects: ReceivedObject[];
...
const SEED_PLACES: Place[] = [
  ...
  { id: 'seed-place-1', name: 'Café Orée', category: 'cafe', personalFit: 'kept', impression: 'Quiet corner, easy to stay.', createdAt: '2026-02-10T10:00:00Z', sourceRelationId: '6', quickSignal: { repeatDesire: true, contextFit: ['deep_talk', 'calm'], landingLevel: 4 }, wentAgainAt: '2026-03-12T11:00:00Z' },
  ...
  { id: 'seed-place-4', name: 'Maison Luma', category: 'restaurant', personalFit: 'kept', impression: 'Warm dinner spot with a calm rhythm.', createdAt: '2026-01-28T20:00:00Z', sourceRelationId: '6', quickSignal: { repeatDesire: true, shareSafe: true, contextFit: ['deep_talk', 'calm'], driverDimensions: ['food', 'atmosphere'], restaurantDimensions: { food: 5, service: 4, atmosphere: 3, value: 4, cleanliness: 2 }, landingLevel: 4 } },
  ...
  { id: 'seed-place-6', name: 'Atelier Nord', category: 'restaurant', personalFit: 'kept', impression: 'Simple menu, strong sense of place.', createdAt: '2026-04-02T19:30:00Z', sourceRelationId: '10', quickSignal: { driverDimensions: ['food'], landingLevel: 4 } },
  ...
  { id: 'seed-place-7', name: 'Le Fond du Couloir', category: 'bar', personalFit: 'kept', impression: 'Found on my own. Good enough to return.', createdAt: '2026-01-15T22:00:00Z', worldFit: ['creative'], quickSignal: { landingLevel: 4 }, wentAgainAt: '2026-02-20T21:00:00Z' },
  ...
  { id: 'seed-place-11', name: 'Café Long', category: 'cafe', personalFit: 'kept', impression: 'Works for long sessions.', createdAt: '2026-04-12T10:00:00Z', sourceRelationId: '13', quickSignal: { contextFit: ['work_focus', 'calm'], landingLevel: 3 } },
  // No sourceRelationId — kept with two worldFit worlds → proves max-2 direct qualification
  { id: 'seed-place-12', name: 'Studio Lent', category: 'spot', personalFit: 'kept', impression: 'Off-hours, no pressure, good for tinkering.', createdAt: '2026-04-20T15:00:00Z', worldFit: ['sport', 'culture'], quickSignal: { contextFit: ['work_focus', 'calm'] } },
];
// Seed received objects for device checks — linked to revealed relations and existing places.
// status: 'new' so Home prompt is visible on first dev run.
const SEED_RECEIVED_OBJECTS: ReceivedObject[] = [
  {
    id: 'recv-seed-1',
    createdAt: '2026-06-20T10:00:00Z',
    objectType: 'place',
    objectId: 'seed-place-4',     // Maison Luma — already in SEED_PLACES
    fromRelationId: '8',           // Camille — revealed
    nameSnapshot: 'Maison Luma',
    categorySnapshot: 'restaurant',
    note: 'Thought of you for a slow dinner.',
    status: 'new',
  },
  {
    id: 'recv-seed-2',
    ...
    objectType: 'place',
    objectId: 'recv-place-new-1',  // does not exist in SEED_PLACES → Keep must create it
    fromRelationId: '10',          // Sophie — revealed
...
const state: StoreState = {
  ...
  passedObjects: [],
  receivedObjects: __DEV__ ? SEED_RECEIVED_OBJECTS : [],
...
export function getEvaluationsSnapshot() {
  return state.evaluations;
}

// Exported for test access and low-level store validation only.
// Application UI should go through useRelationsStore(). Same applies to
// appendPlaceRead and resetDevStateToSeed below.
export function getPlacesSnapshot() {
  return state.places;
}

export function getPassedObjectsSnapshot() {
  return state.passedObjects;
}

export function getReceivedObjectsSnapshot() {
...
export function sanitizePersistedPassedObjects(value: unknown): PassedObject[] {
  if (!Array.isArray(value) || value.length === 0) return [];
  return value.filter(
    (p): p is PassedObject =>
      p !== null &&
      typeof p === 'object' &&
      typeof (p as PassedObject).id === 'string' &&
      typeof (p as PassedObject).createdAt === 'string' &&
      (p as PassedObject).objectType === 'place' &&
      typeof (p as PassedObject).objectId === 'string' &&
      typeof (p as PassedObject).toRelationId === 'string' &&
      PLACE_CATEGORIES.includes((p as PassedObject).categorySnapshot),
  );
}

const RECEIVED_OBJECT_STATUSES: ReceivedObjectStatus[] = ['new', 'kept', 'not_for_me'];

// Exported for regression testing — not part of the public store API.
export function sanitizePersistedReceivedObjects(value: unknown): ReceivedObject[] {
  if (!Array.isArray(value) || value.length === 0) return [];
  const result: ReceivedObject[] = [];
  for (const r of value) {
    if (
      r === null ||
      typeof r !== 'object' ||
      typeof (r as ReceivedObject).id !== 'string' ||
      typeof (r as ReceivedObject).createdAt !== 'string' ||
      (r as ReceivedObject).objectType !== 'place' ||
      typeof (r as ReceivedObject).objectId !== 'string' ||
      typeof (r as ReceivedObject).fromRelationId !== 'string' ||
      typeof (r as ReceivedObject).nameSnapshot !== 'string' ||
      (r as ReceivedObject).nameSnapshot.trim().length === 0 ||
      !PLACE_CATEGORIES.includes((r as ReceivedObject).categorySnapshot) ||
      !RECEIVED_OBJECT_STATUSES.includes((r as ReceivedObject).status)
    ) {
      continue;
    }
    const raw = r as Record<string, unknown>;
    // Legacy migration: fromPassId → fromDeliveryId; new field takes precedence
    const fromDeliveryId: string | undefined =
      typeof raw['fromDeliveryId'] === 'string' && (raw['fromDeliveryId'] as string).length > 0
        ? raw['fromDeliveryId'] as string
        : typeof raw['fromPassId'] === 'string' && (raw['fromPassId'] as string).length > 0
          ? raw['fromPassId'] as string
          : undefined;
    result.push({
      ...(r as ReceivedObject),
      ...(fromDeliveryId !== undefined ? { fromDeliveryId } : {}),
    });
  }
  return result;
}
...
function persist() {
  ...
  persistState<PersistedState>({
    ...
    places: state.places,
    passedObjects: state.passedObjects,
...
export function applyHydratedState(persisted: unknown): void {
  ...
  if (
    ...
    // passedObjects: added in X.81 — missing in older installs → start empty.
    state.passedObjects = sanitizePersistedPassedObjects(
      (persisted as Record<string, unknown>).passedObjects,
    );
    // receivedObjects: added in X.82 — missing in older installs → start with seed.
    const persistedReceived = sanitizePersistedReceivedObjects(
      (persisted as Record<string, unknown>).receivedObjects,
    );
    state.receivedObjects = persistedReceived.length > 0
      ? persistedReceived
      : (__DEV__ ? SEED_RECEIVED_OBJECTS : []);
  } else {
...
const RESTAURANT_DIMENSION_CATEGORIES: PlaceCategory[] = ['restaurant', 'cafe', 'bar'];

/**
 * Appends a single, immutable read to a place — never edits, never removes,
 * never touches any other field (quickSignal/impression/personalFit/
 * wentAgainAt/sourceRelationId/worldFit all stay exactly as they were).
 * Deliberately bypasses mergePlaceUpdate/setPlace — those exist to replace
 * whole-object fields, which is exactly the overwrite behavior a read must
 * never have. id/createdAt/categorySnapshot/criteriaVersion are always
 * generated here, never accepted from the caller.
 */
export function appendPlaceRead(placeId: string, input: PlaceReadEntryInput): boolean {
  const place = state.places.find((item) => item.id === placeId);
  if (!place) return false;

  const landingLevel = sanitizePlaceLandingLevel(input.landingLevel);
  const contextFit = sanitizePlaceContextFit(input.contextFit);
  const driverDimensionsAllowed = RESTAURANT_DIMENSION_CATEGORIES.includes(place.category);
  const driverDimensions = driverDimensionsAllowed
    ? sanitizePlaceDriverDimensions(input.driverDimensions)
    : undefined;
  const restaurantDimensions =
    driverDimensions !== undefined && driverDimensions.length > 0
      ? sanitizeRestaurantExperienceDimensions(input.restaurantDimensions)
      : undefined;
  const cleanImpression = input.impression?.trim();

  const entry: PlaceReadEntry = {
    id: `read-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
    createdAt: new Date().toISOString(),
    ...
    ...(cleanImpression ? { impression: cleanImpression } : {}),
  };
  ...
  state.places = state.places.map((item) => {
    ...
    didUpdate = true;
    return { ...item, reads: [...(item.reads ?? []), entry] };
  });
...
const PASS_NOTE_MAX = 80;

/**
 * Appends a single, immutable pass event — never edits, never removes.
 * id/createdAt/objectType are always generated here; caller cannot inject them.
 * Does not touch relation trust, route usefulness, taste affinity, or object value.
 */
export function addPassedObject(input: PassedObjectInput): PassedObject {
  const trimmedNote = input.note?.trim();
  const note =
    trimmedNote && trimmedNote.length > 0 ? trimmedNote.slice(0, PASS_NOTE_MAX) : undefined;

  const entry: PassedObject = {
    id: `pass-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
    createdAt: new Date().toISOString(),
    objectType: 'place',
    objectId: input.objectId,
    toRelationId: input.toRelationId,
    categorySnapshot: input.categorySnapshot,
    ...(input.sourceRelationId !== undefined ? { sourceRelationId: input.sourceRelationId } : {}),
    ...(note !== undefined ? { note } : {}),
  };

  state.passedObjects = [...state.passedObjects, entry];
  emitChange();
  persist();
  return entry;
}

const RECEIVED_NOTE_MAX = 80;

/**
 * Appends a single received object — never edits, never removes.
 * id/createdAt/objectType/status are always generated here; caller cannot inject them.
 * Does not touch relation trust, route usefulness, taste affinity, or object value.
 * No read receipt. No delivery confirmation. No scoring.
 */
export function addReceivedObject(input: ReceivedObjectInput): ReceivedObject {
  const trimmedName = input.nameSnapshot.trim() || 'Untitled place';
  const trimmedNote = input.note?.trim();
  const note =
    trimmedNote && trimmedNote.length > 0
      ? trimmedNote.slice(0, RECEIVED_NOTE_MAX)
      : undefined;

  const entry: ReceivedObject = {
    id: `recv-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
    createdAt: new Date().toISOString(),
    objectType: 'place',
    objectId: input.objectId,
    fromRelationId: input.fromRelationId,
    nameSnapshot: trimmedName,
    categorySnapshot: input.categorySnapshot,
    status: 'new',
    ...(input.fromDeliveryId !== undefined ? { fromDeliveryId: input.fromDeliveryId } : {}),
    ...(input.sourceRelationId !== undefined ? { sourceRelationId: input.sourceRelationId } : {}),
    ...(note !== undefined ? { note } : {}),
  };

  state.receivedObjects = [...state.receivedObjects, entry];
  emitChange();
  persist();
  return entry;
}
...
/**
 * Transitions a received object from 'new' to 'kept' or 'not_for_me'.
 * Transitions are irreversible in V0 (kept → not_for_me and reverse are blocked).
 * When kept: creates a minimal local Place (personalFit: 'saved') if objectId unknown.
 * Does not touch relation trust. Does not modify passedObjects. No read receipt.
 */
export function setReceivedObjectStatus(
  id: string,
  status: 'kept' | 'not_for_me',
): ReceivedObject | undefined {
  const entry = state.receivedObjects.find((r) => r.id === id);
  if (!entry) return undefined;
  // Only allow new → kept or new → not_for_me.
  if (entry.status !== 'new') return undefined;

  const decidedAt = new Date().toISOString();
  const updated: ReceivedObject = { ...entry, status, decidedAt };
  state.receivedObjects = state.receivedObjects.map((r) => (r.id === id ? updated : r));

  // Keep: materialize a minimal Place if objectId is not already in the store.
  if (status === 'kept' && entry.objectType === 'place') {
    const alreadyExists = state.places.some((p) => p.id === entry.objectId);
    if (!alreadyExists) {
      const minimalPlace: Place = {
        ...
        personalFit: 'saved',
        createdAt: new Date().toISOString(),
        sourceRelationId: entry.fromRelationId,
      };
      state.places = [...state.places, minimalPlace];
    }
    // Place already exists: leave it untouched regardless of current personalFit.
  }
...
/**
 * Materializes remote pass deliveries as local ReceivedObjects.
 * Idempotent: skips deliveries already present by fromDeliveryId.
 * Skips deliveries whose canonicalRelationId has no matching local relation.
 * Does not modify passedObjects, evaluations, or relation trust.
 * sourceRelationId is never materialized from delivery payload.
 */
export function materializePassDeliveries(
  deliveries: PassDeliveryMaterializationInput[],
): ReceivedObject[] {
  if (!deliveries.length) return [];

  const created: ReceivedObject[] = [];

  for (const delivery of deliveries) {
    // Idempotency: skip if already materialized by this delivery id
    if (state.receivedObjects.some((r) => r.fromDeliveryId === delivery.fromDeliveryId)) {
      continue;
    }
    // Resolve local relation by canonicalRelationId
    const localRelation = state.relations.find(
      (r) => r.canonicalRelationId === delivery.canonicalRelationId,
    );
    if (!localRelation) continue;

    const trimmedName = delivery.objectPayload.nameSnapshot.trim() || 'Untitled place';
    const trimmedNote = delivery.objectPayload.note?.trim();
    const note =
      trimmedNote && trimmedNote.length > 0
        ? trimmedNote.slice(0, RECEIVED_NOTE_MAX)
        : undefined;
    ...
    const entry: ReceivedObject = {
      ...
      createdAt: new Date().toISOString(),
      objectType: 'place',
      objectId: delivery.objectPayload.objectId,
      fromRelationId: localRelation.id,
      fromDeliveryId: delivery.fromDeliveryId,
      nameSnapshot: trimmedName,
      categorySnapshot: delivery.objectPayload.categorySnapshot,
      status: 'new',
      ...(note !== undefined ? { note } : {}),
      // sourceRelationId intentionally absent — never materialized from delivery
    };
...
export function resetDevStateToSeed() {
  ...
  state.passedObjects = [];
  state.receivedObjects = [...SEED_RECEIVED_OBJECTS];
  state.progressivePrivateSignals = {};
  hydrated = true;
  emitChange();
...
export function useRelationsStore() {
  ...
  const passedObjects = useSyncExternalStore(
    subscribe,
    getPassedObjectsSnapshot,
    getPassedObjectsSnapshot,
  );
  const receivedObjects = useSyncExternalStore(
    subscribe,
  ...
  const addPlaceRead = (placeId: string, input: PlaceReadEntryInput) =>
    appendPlaceRead(placeId, input);
  const addPassObject = (input: PassedObjectInput) => addPassedObject(input);
  const addReceivedObject_ = (input: ReceivedObjectInput) => addReceivedObject(input);
  const setReceivedObjectStatus_ = (id: string, status: 'kept' | 'not_for_me') =>
    setReceivedObjectStatus(id, status);
  ...
  return {
    ...
    addPlaceRead,
    addPassObject,
    addReceivedObject: addReceivedObject_,
    setReceivedObjectStatus: setReceivedObjectStatus_,
    resolveInvitedSideB,
    startCookingReveal,
